...
App Store Connect 的截屏槽位会随账号/App 状态显示不同世代的尺寸，**以页面上标注的为准**。
四种尺寸都已生成好，直接选对应目录上传即可：

| ASC 槽位标注 | 像素 | 目录 |
|---|---|---|
| iPhone 6.5 英寸显示屏 | 1284 × 2778 | `zh/` `en/` |
| iPhone 6.9 英寸显示屏 | 1320 × 2868 | `zh-6.9/` `en-6.9/` |
| iPad 12.9 英寸显示屏 | 2048 × 2732 | `zh-ipad/` `en-ipad/` |
| iPad 13 英寸显示屏 | 2064 × 2752 | `zh-ipad13/` `en-ipad13/` |

> 6.5″ 槽位也接受 1242×2688，但 1284×2778 分辨率更高，优先用它。
> 需要其它尺寸时：`./make_shots <rawDir> <outDir> <zh|en> <canvas>`，
> canvas 取 `iphone65` / `iphone69` / `ipad129` / `ipad13`。
...
| App 信息 → 类别 | 效率（主）／商务（次） |
| 价格与销售范围 | [定价] |
| 1.0 准备提交 → 截屏（iPhone） | **先看 ASC 页面标的尺寸**：6.5″（1284×2778）用 `zh/` `en/`；6.9″（1320×2868）用 `zh-6.9/` `en-6.9/` |
| 1.0 准备提交 → 截屏（iPad） | 12.9″（2048×2732）用 `zh-ipad/` `en-ipad/`；13″（2064×2752）用 `zh-ipad13/` `en-ipad13/` |
